...
def custom_loss(y_pred, y_true):        # WCE
        ...
        numerator = tf.math.multiply(2., tf.math.add(tf.reduce_sum(tf.math.multiply(y_true, y_pred), axis=[0, 1, 2]), 1e-7))
        denominator = tf.math.add(tf.reduce_sum(tf.math.add(y_true, y_pred), axis=[0, 1, 2]), 1e-7)
        dice = tf.math.divide(tf.math.multiply(weights, numerator), tf.math.multiply(weights, denominator))
