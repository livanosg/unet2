...
def weighted_crossentropy(predictions, labels):
    ...
    with tf.name_scope('Weighted_Crossentropy'):
        ...
        weights = tf.math.divide(tf.reduce_sum(class_frequencies)-class_frequencies, tf.add(class_frequencies, epsilon()))
        wce = tf.reduce_sum(weights * labels * tf.math.log(predictions + epsilon()))
        wce = tf.negative(tf.math.divide(wce, tf.reduce_sum(class_frequencies)))
        return wce
...
def weighted_log_dice_loss(predictions, labels):
    ...
    with tf.name_scope('Generalized_Dice_Loss'):
        ...
        loss = tf.negative(tf.math.log(tf.multiply(tf.math.divide(numerator, denominator), 2.0)))
    return loss


def custom_loss(y_pred, y_true):
    with tf.name_scope('Custom_loss'):
        dice_loss = weighted_log_dice_loss(labels=y_true, predictions=y_pred)
        xe_loss = weighted_crossentropy(labels=y_true, predictions=y_pred)
